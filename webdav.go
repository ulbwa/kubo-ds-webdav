...
func (d *Datastore) start(ctx context.Context, explicitShard bool) error {
	...
	if err := d.c.mkcolAll(ctx, ""); err != nil {
		return fmt.Errorf("webdavds: cannot create root at %s: %w", d.c.urlBase, err)
	}
	if err := d.c.mkcolAll(ctx, "blocks"); err != nil {
		return fmt.Errorf("webdavds: cannot create blocks collection: %w", err)
	}

	// Capability probe: decide the write strategy.
	...
	if d.c.cfg.UseMove != nil {
		...
		d.useMove = strings.Contains(strings.ToUpper(allow), "MOVE")
	}
...
// Put writes a value. For content-addressed blocks this is idempotent.
func (d *Datastore) Put(ctx context.Context, k ds.Key, value []byte) error {
	dir, full := d.pathFor(k)
	if dir != "" {
		if err := d.c.mkcolAll(ctx, dir); err != nil {
			return err
		}
	}
	if d.useMove {
	...
	} else if err := d.c.put(ctx, full, value); err != nil {
		return err
	}
...
// Depth:1) and applies filters/orders/offset/limit naively, so GC and
// `ipfs repo` work without the backend having to support rich queries.
func (d *Datastore) Query(ctx context.Context, q dsq.Query) (dsq.Results, error) {
	var entries []dsq.Entry
	startDir := strings.Trim(q.Prefix, "/")
	if err := d.walk(ctx, startDir, q.KeysOnly, &entries); err != nil {
		return nil, err
	}
	res := dsq.ResultsWithEntries(q, entries)
	return dsq.NaiveQueryApply(q, res), nil
}

// walk recursively collects file entries (as datastore keys) under a
// key-relative directory.
func (d *Datastore) walk(ctx context.Context, dir string, keysOnly bool, out *[]dsq.Entry) error {
	...
	for _, it := range items {
		if it.IsDir {
			if err := d.walk(ctx, it.Rel, keysOnly, out); err != nil {
				return err
			}
			continue
		}
		...
		ent := dsq.Entry{Key: key.String(), Size: int(it.Size)}
		if !keysOnly {
			v, gerr := d.Get(ctx, key)
			if gerr != nil {
				if errors.Is(gerr, ds.ErrNotFound) {
...
func (d *Datastore) keyForRel(rel string) (ds.Key, bool) {
	...
	if rel == "" || rel == shardingKey {
		return ds.Key{}, false
	}
	if strings.HasPrefix(rel, ".tmp-") || strings.Contains(rel, "/.tmp-") {
		return ds.Key{}, false // in-flight temp file
	}
